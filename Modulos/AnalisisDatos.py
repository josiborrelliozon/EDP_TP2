...
if __name__=='__main__':
    ...
    #filtro las 20 mas instaladas, eje x precio, eje y rating, el tamaño segun el tamaño
    #installs = [0]*len(Installs)
    #for i in range(len(Installs)):
    #    installs = (Installs[i])[:-1].replace(",", "").replace('','0')
    ...
    filtro = [x for x in apps_ratings if not math.isnan(float(x[1])) and x[2] != 'Varies with device'] #filtro las q tienen rating y el tamaño no varia
    dow =[]
    ratin = []
    tamaño = []
    apli = []
    for i in range(len(filtro)):
        ...
        dow.append(float(filtro[i][0]))
        apli.append(filtro[i][3])
        tamaño.append(float(filtro[i][2][:-1]))
    print(ratin)
    print(dow)
    print(tamaño)
    bubble_size = np.array(dow) * 10
    ...
    #    plt.text(tamaño[i] + 0.05, ratin[i], apli[i], fontsize=9)
    plt.show()
